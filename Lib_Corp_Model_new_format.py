...
def run_EBS(valDate, eval_date, work_EBS, Scen, liab_summary, EBS_asset, AssetAdjustment, SFS_BS, market_factor):
    ...
    asset_mv_acc_int_summary = EBS_asset.groupby(['Fort Re Corp Segment'])['accrued interest usd'].agg('sum')
    ...
    for each_account in accounts:
        ...
        if each_account == 'LT':
            ...
            if isinstance(AssetAdjustment, pd.DataFrame): ### for actual
                ...
                work_EBS[each_account].LOC                   = UI.EBS_Inputs[valDate][each_account]['LOC']*(eval_date < datetime.datetime(2020,6,1))
